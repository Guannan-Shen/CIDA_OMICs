...
```{r permutation}
## get all numeric 
ai_com <- as.matrix(apodata)
N <- choose(16, 8)
## get the complement vector
'%nin%' <- Negate('%in%')
## put the long vector at the first place 
ai_com[1,][ai_com[1,] %nin% gene1_com[1,] ]

## get the combinations for gene1
gene1_com <- combinations(16, 8, ai_com[1,], set = FALSE)
dim(gene1_com)

## get t
gene1_paired_t <- apply(gene1_com, 1, function(gene1_com){
   ai_com[1,][ai_com[1,] %nin% gene1_com]
})

gene1_paired <- t(gene1_paired_t)

## test of the combination and its complement
ai_com[1, ] %in% c(gene1_com[2, ], gene1_paired[2, ])
...
gene1_t <- sapply(1:6384, function(row){
  test = t.test(gene1[row, 1:8], ai[row, 9:16], alternative = "two.sided")
  test$statistic
})
## compare the t statistic and get the p-value
p_gene1 <- sum(abs(gene1_t) > abs(indi_t$tstatistic[1]))/N
p_gene1
